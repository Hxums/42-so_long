...
/*   Updated: 2025/12/10 14:45:46 by hcissoko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// because 1024 is the maximum of file that can be open
// at the same time
char	*get_next_line(int fd)
{
	...
	while (0 < bytes_read)
	{
		if (ft_strchr(temp[fd], '\n'))
			return (ft_getline(&temp[fd], &buffer, bytes_read));
		...
		if (bytes_read < 1)
			break ;
		buffer[bytes_read] = '\0';
		temp[fd] = ft_affect_temp(temp[fd], buffer);
		free(buffer);
		buffer = NULL;
	}
	return (ft_getline(&temp[fd], &buffer, bytes_read));
